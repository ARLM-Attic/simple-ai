...
namespace SimpleEngine.Core
{
    ...
    /// </summary>
    public class DynamicLineBatch : LineBatch
    {
        #region Constants
        /// <summary>
        ...
        public DynamicLineBatch(GraphicsDevice graphicsDevice)
        {
        ...
        public void DrawCylinder(Vector3 basePosition, float radius, float height, int slicesCount, Color color)
        {
            // sanity check
            if (slicesCount < 3)
            {
            ...
            endTop.X += radius;
            endTop.Z += height;
            endBottom = basePosition;
            endBottom.X += radius;

            for (int index = 1; index < (slicesCount + 1); ++index)
            {
                startTop = endTop;
                startBottom = endBottom;
                float radians = (2.0f * MathHelper.Pi / slicesCount) * index;
                endTop = basePosition;
                endTop.X += (float)Math.Cos((double)radians) * radius;
                endTop.Y += (float)Math.Sin((double)radians) * radius;
                endTop.Z += height;
                endBottom = basePosition;
                endBottom.X = endTop.X;
                endBottom.Y = endTop.Y;

                DrawLine(startTop, endTop, color);
                DrawLine(startBottom, endBottom, color);
                DrawLine(endTop, endBottom, color);
